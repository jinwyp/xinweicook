...
#  app.use libs.secure.middleware
  ...
  app.post("/api/orders/payment/weixinpay/config", orderController.getWeixinDeveloperJsapiTicket) ##去掉libs.auth("member"), 获取jssdk config对匿名用户也应当可以使用, 否则无法对匿名用户在列表页进行定位.
